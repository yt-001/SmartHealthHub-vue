...
const fetchList = async () => {
  ...
  try {
    const resp = await fetchArticleReviewList(buildPayload() as any)
 ...
 * @param id 文章ID
 */
const goDetail = (id: number | string) => { router.push({ path: '/portal/admin/article-edit', query: { id, mode: 'view' } }) }
/**
 * 跳转管理员编辑文章
 * @param id 文章ID
 */
const goEdit = (id: number | string) => { router.push({ path: '/portal/admin/article-edit', query: { id, mode: 'edit' } }) }

onMounted(fetchList)
</script>

<style scoped>
.article-list-page { padding: 16px; }
