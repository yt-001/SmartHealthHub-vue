  ...
  <div class="page-wrap">
    <div class="page-header">
      <div>
        ...
        <p class="page-desc">维护文章分类字典，用于医生发布与筛选</p>
      </div>
...
import type { ArticleCategoriesVO, ArticleCategoriesQuery, ArticleCategoriesCreateDTO, ArticleCategoriesUpdateDTO } from '@/api/types/articleTypes'
import { fetchArticleCategoriesPage, createArticleCategory, updateArticleCategory, deleteArticleCategory } from '@/api/modules/article'

// 分页与列表状态
const loading = ref(false)
const list = ref<ArticleCategoriesVO[]>([])
const total = ref(0)
const pageNum = ref(1)
...
// 查询表单
const queryForm = reactive<ArticleCategoriesQuery>({ name: '', isEnabled: undefined })

// 弹窗与表单
const dialog = reactive<{ visible: boolean; mode: 'create' | 'edit'; currentId?: number }>({ visible: false, mode: 'create' })
const formRef = ref<FormInstance | null>(null)
const form = reactive<ArticleCategoriesCreateDTO & Partial<ArticleCategoriesUpdateDTO>>({ name: '', description: '', sortOrder: 0, isEnabled: 1 })
...
async function fetchPage() {
  ...
  loading.value = true
  try {
    const resp = await fetchArticleCategoriesPage({ pageNum: pageNum.value, pageSize: pageSize.value, sortField: 'sortOrder', sortDirection: 'ASC', query: { name: queryForm.name || undefined, isEnabled: queryForm.isEnabled } })
 ...
 * @param row 当前行
 */
function openEditDialog(row: ArticleCategoriesVO) {
  dialog.visible = true
  dialog.mode = 'edit'
...
async function onSubmit() {
  ...
  try {
    if (dialog.mode === 'create') {
      await createArticleCategory({ name: form.name!, description: form.description || '', sortOrder: form.sortOrder ?? 0, isEnabled: form.isEnabled ?? 1 })
      ElMessage.success('创建成功')
    } else {
      await updateArticleCategory({ id: dialog.currentId!, name: form.name, description: form.description, sortOrder: form.sortOrder, isEnabled: form.isEnabled })
 ...
 * @param row 当前行
 */
async function onDelete(row: ArticleCategoriesVO) {
