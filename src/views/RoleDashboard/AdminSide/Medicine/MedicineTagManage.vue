...
const formData = reactive<Partial<MedicineTagCreateDTO & { id?: number }>>({
  name: '',
  description: '',
  sortOrder: 0,
  isEnabled: 1
...
const onEdit = (row: MedicineTagVO) => {
  isEdit.value = true
  resetForm()
    ...
    if (valid) {
      ...
      try {
        if (isEdit.value && formData.id != null) {
          await medicineApi.updateMedicineTag(formData as MedicineTagUpdateDTO)
          ElMessage.success('更新成功')
        } else {
          await medicineApi.createMedicineTag(formData as MedicineTagCreateDTO)
...
const onDelete = (row: MedicineTagVO) => {
  ElMessageBox.confirm('确定要删除该标签吗？', '提示', {
    ...
    try {
      await medicineApi.deleteMedicineTag(row.id)
