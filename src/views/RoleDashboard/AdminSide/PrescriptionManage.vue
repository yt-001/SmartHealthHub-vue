    ...
    <el-card shadow="never" class="list-card">
      ...
      >
        ...
        <el-table-column label="操作" width="120" fixed="right" align="center">
          <template #default="{ row }">
            <el-popconfirm title="确定要删除该处方吗？" @confirm="handleDelete(row)">
              <template #reference>
    ...
    <el-dialog v-model="detailVisible" title="处方详情" width="700px">
      <div v-if="currentDetail" class="detail-content">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="诊断结果" :span="2">{{ currentDetail.diagnosis }}</el-descriptions-item>
          <el-descriptions-item label="开方时间" :span="2">{{ currentDetail.createdAt }}</el-descriptions-item>
          <el-descriptions-item label="医嘱/备注" :span="2">{{ currentDetail.notes || '无' }}</el-descriptions-item>
        </el-descriptions>

        <div class="section-title">药品明细</div>
        <el-table :data="parsedMedications" border stripe size="small">
...
const handleDelete = async (row: PrescriptionItem) => {
  try {
    const res = await prescriptionApi.delete(row.id)
    if (res.code === 200) {
